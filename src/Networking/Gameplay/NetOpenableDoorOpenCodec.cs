...
{
    internal static class NetOpenableDoorOpenCodec
    {
        private const byte Version = 1;

        public static void Write(NetDataWriter w, NetOpenableDoorOpen m)
        {
            w.Put(Version);
            w.Put(m.PeerId ?? "");
            w.Put(m.ChapterName ?? "");
        ...
        public static bool TryRead(NetDataReader r, out NetOpenableDoorOpen m)
        {
            m = new NetOpenableDoorOpen();
            try
            {
                byte ver = r.GetByte();
