...
{
    internal static class NetDoorBlockerOpenCodec
    {
        private const byte Version = 1;

        public static void Write(NetDataWriter w, NetDoorBlockerOpen m)
        {
            w.Put(Version);
            w.Put(m.PeerId ?? "");
            w.Put(m.ChapterName ?? "");
        ...
        public static bool TryRead(NetDataReader r, out NetDoorBlockerOpen m)
        {
            m = new NetDoorBlockerOpen();
            try
            {
                byte ver = r.GetByte();
