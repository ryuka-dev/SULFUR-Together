...
- Optional shared world loot, and host-gated developer mode (session vote), as host session settings
- Endless Mode co-op — a shared arena, waves, and enemies, with independent or shared progression (a group card vote)
- End-of-run stats cards and join/leave notifications
- 14-language interface

### Saves, inventory, and loot

SULFUR Together does **not** use a shared character save. Each player keeps their own character, inventory, equipment, progression, personal loot, and save data. Items you deliberately drop into the world can be picked up by others. **Optional shared loot** is available too — the host can turn on *Shared loot* on the connect page, which shares enemy, crate, chest, and hatbox/register loot (first player to grab each item takes it). It is **off by default**; otherwise each player gets their own loot as before.
...
- In Shared Endless Mode, a level-up card can occasionally show the wrong art or text on one screen (the vote and the reward are still correct).
...
- 友军伤害（友伤）会话开关（由房主决定）
- 可选的共享世界掉落，以及需全员授权或投票开启的开发者模式（均为房主会话设置）
- 无尽模式联机——共享的竞技场、波次与敌人，进度可选独立或共享（组队选卡投票）
- 结算统计卡片与加入/离开通知
- 14 种语言界面

### 存档、背包与掉落

SULFUR Together **不使用**共享角色存档。每位玩家保留自己的角色、背包、装备、进度、个人掉落与存档数据。你**主动**丢到世界里的物品可以被其他人捡起。现在还提供**可选的共享掉落**——房主可在连接页开启 *Shared loot（共享掉落）*，此时敌人、木箱、宝箱以及帽盒/收银机的战利品都会共享（谁先捡到归谁）。该功能**默认关闭**；关闭时每位玩家和以前一样获得各自的掉落。
...
- 共享无尽模式下，升级选卡偶尔会在某一方屏幕上显示错误的图标或文字（投票与获得的奖励始终正确）。
- 部分敌人可能出现瞬移、抽动或动画错误。
...
- 任意の共有ワールド戦利品と、全員の権限または投票で有効化する開発者モード（いずれもホストのセッション設定）
- エンドレスモードの協力プレイ——アリーナ・ウェーブ・敵を共有し、進行は独立または共有（合同カード投票）を選択可能
- リザルト統計カードと参加／退出の通知
- 14言語対応のインターフェース

### セーブ・インベントリ・戦利品

SULFUR Together は共有キャラクターセーブを**使いません**。各プレイヤーは自分のキャラクター・インベントリ・装備・進行・個人戦利品・セーブデータを保持します。あなたが**意図的に**ワールドへ落としたアイテムは他のプレイヤーが拾えます。さらに**任意の共有戦利品**も利用できます——ホストが接続ページで *Shared loot（共有戦利品）* をオンにすると、敵・木箱・宝箱・帽子箱／レジの戦利品が共有されます（各アイテムを最初に拾った人が入手）。**既定ではオフ**で、オフの場合は従来どおり各プレイヤーが自分の戦利品を得ます。
...
- 共有エンドレスモードでは、レベルアップ時のカードが片方の画面で誤ったアートやテキストで表示されることがあります（投票と獲得する報酬は常に正しい）。
