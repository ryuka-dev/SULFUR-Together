...
## 5. `CousinHelper`
1. **Start entry:** `Trigger()`→`TriggerIntro()` (sets `triggeredByPlayer`); dialog/anim → `Introduction()` (`introPlayed=true`, teleport, camera, animator "Intro") → `StartFight()` (`FightStarted=true`, unlock, `EnablePoolDamagers(true)`).
2. **Dialog entry:** cousin NPC dialog → `Trigger` (ITriggerable).
3. **Main health Unit:** `owner` (`GetComponent<Unit>()`).
4. **Bar attach:** `owner` — **NOTE: `StartFight` does NOT call AttachToBossUI**; the boss bar attach for Cousin is NOT in the start chain (E4.2 manager now force-attaches `owner`). Need to confirm the real attach point (likely an animation event).
