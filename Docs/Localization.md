...
|---|---|---|---|---|
| 1 | `Press [{key}] to enter the arena` | `arena.enterPrompt` | `ArenaLockdownManager.cs` (LD-2c popup banner, via `ShowPrompt`) | `{key}` = `ArenaEnterConfirmKey`. Shown to the out-of-room player 5 s after the seal (LD-2h). |
...
| 4 | title `Arena Lockdown` / msg `A teammate entered the arena — head in now to join them!` | `arena.toast.title`, `arena.toast.teammateEntered` | `ArenaLockdownManager.cs` (LD-2c `Notify` toast, t0, via `ShowToast`) | Heads-up at t0; LD-2d/2h grace keeps the door open until everyone is in (cap 15 s), so it invites the player to run in. |
| 5 | title `Arena Lockdown` / msg `You've been sealed out — you'll be brought in shortly.` | `arena.toast.title`, `arena.toast.sealedOut` | `ArenaLockdownManager.cs` (LD-2c `Seal` toast, fires at the LD-2h seal) | Explains the otherwise-invisible barrier. |
...
| 33 | Join-attempt toast — `Connecting to host…` | `toast.connectingToHost` | `NetConnectFeedback.BeginAttempt` (TOAST-JoinFlow) | Fired once per connect attempt (false→true transition; the Steam path calls BeginAttempt twice) on every join route — Direct IP, Steam-ID, and invite accept (which previously showed nothing until the handshake resolved). Success is announced by the existing `toast.connectedTo`; the `link.linkedToHost` toast no longer fires during an attempt (row 12 — it stays for the mid-session PageDown/PageUp toggles). |
