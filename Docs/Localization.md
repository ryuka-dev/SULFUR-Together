...
| 31 | Shared card-vote status hint — pre-vote prompt, waiting line, resolved line | `endless.cardvote.prompt`, `endless.cardvote.waiting`, `endless.cardvote.resolved` | `EndlessCardVoteOverlay` (EM-6b-3a) | Bottom-centre line under the shared 1-of-N card vote. `{voted}`/`{total}`/`{secs}` tokens on the waiting line (countdown only runs after the first cast); `{index}` (1-based) on the resolved line. On-card voter *names* are player-supplied, not localized. |
