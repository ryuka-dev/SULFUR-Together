...
| **RT3-Cousin-arms client-visual-all-balls** | Client only sees the ball aimed at its local player (ghost proxies are host-only, so client `GameManager.Players` = local only). Host AoE damage is complete; this is a client-only visual gap (would throw extra visual balls at remote-player proxy positions). | open (low priority) |
