...
| **RM-2b** | **Boss intro cutscene scoped to in-room** (Cousin): out-of-room ends don't play the dialog/camera-turn/Cinematic-lock (suppressed via flag + patches on `ModifyControllerLock`/`ModifyPlayerInvulnerability`/`Player.RotateCameraTowardsPosition` + `Npc.Interact` block) — the boss still **appears** (forced `Introduction` at commit / native intro woken by activation). A late entrant (walk-in or lockdown teleport) **catches up** the dialog (TriggerIntro, or open `Npc.Interact` directly if `introPlayed` already set). Per-encounter dialog session: opened on intro, closed at fight commit (→ dialog removed, no re-open). Config `GateBossDialogToInRoom`. `[BossDialogCutscene]` | verified Log162–166 (out-of-room boss appears + no cutscene; catch-up; in-room normal) | 
