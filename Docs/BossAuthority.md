...
- **Intro-cutscene "native finish" sync — ATTEMPTED, REVERTED (needs a foundation fix first).** Goal (user spec): players read the intro at their own pace; when any player reads to the end, every other end still reading runs the **last step natively** (`OnDialogueFinished` → boss end presentation), instead of the current crude host→client `Graph.Stop` close. Implemented as a finish-event relay + a per-frame `AcceptDialogOption` fast-forward — but it broke (Log297: host dialog re-opened, looped). **Root cause is the foundation, not the sync:** the client applies the boss start via TWO paths — `HostBossEncounterStart` → `TryApplyHostStart` AND the dialog commit → `TryApplyDialogCommit`, and `TryApplyDialogCommit` calls `TryFinalizeCurrentDialog` (`Graph.Stop`) which **force-closes the intro dialog** before re-running `OnStartInteractWithBoss`. That force-close misfires any "the dialog finished" detection, and fast-forwarding fights the still-running intro chain. **Fix the foundation first** (dedup the client's start-application to one path; don't finalize the intro dialog), then redo the native-finish sync on a stable base. Interim: intro close stays `Graph.Stop` (works, not native, one-directional).
